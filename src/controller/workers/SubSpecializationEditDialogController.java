...
public class SubSpecializationEditDialogController {
    ...
    private SubSpecialization obj;
    public static ObservableList<Specialization> groups;
    ...
    public void setData(Object obj) {
        ...
        this.obj = (SubSpecialization) obj;
        dField1.setText(this.obj.getTitle());

        try {
            groups = SubSpecializationOverviewController.tService.get_specialization_list();
            dCombo1.setItems(groups);
            dCombo1.getSelectionModel().select(this.obj.getMain_id());
    ...
    private void handleOk() {
        if (isInputValid()) {
            ...
            for(Specialization item : groups){
                if (item.getTitle() == dCombo1.getSelectionModel().selectedItemProperty().getValue().toString()) {
                    obj.setMain_id(item.getId());
                    obj.setMain_name(item.getTitle());
                }
            }

            try {
                if(obj.getId() != null) {
                    SubSpecializationOverviewController.tService.edit(obj);
                }
                else {
                    SubSpecializationOverviewController.tService.add(obj);
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
